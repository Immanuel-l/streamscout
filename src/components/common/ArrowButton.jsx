...
function ArrowButton({ direction, onClick, groupHoverClass = 'group-hover/row' }) {
  ...
  // Tailwind needs static class names to generate variant styles.
  const hoverOpacityClass = groupHoverClass === 'group-hover/cast'
    ? 'group-hover/cast:opacity-100'
  ...
  return (
    <button
      ...
      className={`absolute top-0 ${isLeft ? 'left-0' : 'right-0'} z-10 h-full w-12 sm:w-14 flex items-center ${isLeft ? 'justify-start' : 'justify-end'} opacity-0 ${hoverOpacityClass} transition-opacity duration-300 cursor-pointer`}
    >
